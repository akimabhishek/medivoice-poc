...
# 1. Initialize the limiter using the client's remote IP address as the key
limiter = Limiter(key_func=get_remote_address)

app = FastAPI()

# 2. Attach the limiter state and custom error handler to the FastAPI app
...
async def process_query(request: Request, text_query: str = Form(...)):
    ...
    try:
        ...
        # --- GUARDRAIL: Max 500 characters ---
        if len(clean_transcript) > 500:
            raise HTTPException(status_code=400, detail="Query is too long. Please keep it under 500 characters.")
        ...
        # Synthesize Audio for Text Queries Too!
...
async def process_voice_query(request: Request, audio_file: UploadFile = File(...)):
    """Handles end-to-end VOICE interactions (STT -> RAG -> Gemini -> TTS)."""
    start_time = time.time()

    MAX_AUDIO_SIZE = 2 * 1024 * 1024  # 2 Megabytes
    if audio_file.size and audio_file.size > MAX_AUDIO_SIZE:
        raise HTTPException(status_code=400, detail="Audio file too large. Please keep voice notes under 30 seconds.")
    ...
    try:
        ...
        # Extract accurate tokens from Gemini
        input_tokens = ai_result.get("usage_metadata", {}).get("prompt_tokens", 0)
        output_tokens = ai_result.get("usage_metadata", {}).get("completion_tokens", 0)
...
async def upload_document(request: Request, background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    # --- GUARDRAIL 1: Must be a PDF or TXT ---
